...
prop.plot <- function(input,title,color,hide.ns,method,legendLocation,stat.test,ct,strip.size=8,ncol=7,plot.margin=margin(0,0,0,2),title.y){
  ...
  #stat.test$y.position[which(stat.test$p.adj.signif < 0.05)] <- c(0.7,0.8,0.5,0.55,0.65,0.5,0.6)
  p1 <- p1 + stat_pvalue_manual(stat.test, label = "p.adj.signif",hide.ns = hide.ns,
                                tip.length = 0,size = 2.5,color = "red");p1
  return(p1)
}

prop.plot.surgical <- function(input,title,color,hide.ns,method,legendLocation,stat.test,ct,ncol=7,strip.size=8,plot.margin=margin(0,0,0,2),title.y){
  input <- cbind(input[,ct],input$`sampleInfo[rownames(prop_ct), 2]`)
  tmp.plot <- melt(input,value.name="Prop",variable.name="cluster");
  colnames(tmp.plot)[1] <- "Groups";dim(tmp.plot)
  ...
  #stat.test$y.position[which(stat.test$p.adj.signif < 0.05)] <- c(0.7,0.8,0.5,0.55,0.65,0.5,0.6)
  p1 <- p1 + stat_pvalue_manual(stat.test, label = "p.adj.signif",hide.ns = hide.ns,
                                tip.length = 0,size = 2.5,color = "red")
  return(p1)
}

prop.fun <- function(bottom,int,ct.epi){
  if(bottom=="all") {
    meta <- ss@meta.data
...
##Example
##Fig3 AC endoscopy; epithelial cell type chagnes
...
prop.plot(input,title = " ",color = c("#B09C85FF", "#00A087FF", "#E64B35FF"),hide.ns = T,method = "fdr",legendLocation = c(.94,.2),stat.test = stat.test,ct=ct.epi,plot.margin = margin(0,2,0,2),strip.size = 6,title.y = "Proportion (AC)");p6
